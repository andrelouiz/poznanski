...
const Cards = ({ item }) => {
  ...
  const handleAddToCart = item => {
    ...
    if(user && user.email){
        ...
        axios.post('https://poznanski.onrender.com/carts', cartItem)
        .then((response) => {
          ...
          if(response){
            refetch(); // refetch cart
              Swal.fire({
                  position: 'center',
                  icon: 'success',
                  title: 'Device added on the cart.',
        ...
        Swal.fire({
            title: 'Please login to order the device',
            icon: 'warning',
  ...
  return (
    <div to={`https://poznanski.onrender.com/menu/${item._id}`} className="card shadow-xl relative mr-5 md:my-5">
    <div
      className={`rating gap-1 absolute right-2 top-2 p-4 heartStar bg-red ${
        isHeartFilled ? "text-rose-500" : "text-white"
      } z-50`}  // Added a high z-index value
