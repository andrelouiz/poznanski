...
const Navbar = () => {
  ...
  const navItems = (
    <>
      <li><a href="/" className={`text-${isDarkMode ? 'white' : 'black'}`}>Home</a></li>
      <li><a href="/menu" className={`text-${isDarkMode ? 'white' : 'black'}`}>Shop Devices</a></li>

      <li tabIndex={0}>
        <details>
          <summary className={`text-${isDarkMode ? 'white' : 'black'}`}>Services</summary>
          <ul style={{ backgroundColor: `${isDarkMode ? "black" : ""}`, color: isDarkMode ? 'white' : 'black' }}>
            <li><a href="/AboutUs">About Us</a></li>
            <li><a href="/FAQ">FAQ</a></li>
            <li><a href="/CCNA">CCNA Training</a></li>
          </ul>
        </details>
      </li>
      <li><a href="/order" className={`text-${isDarkMode ? 'white' : 'black'}`}>Track Order</a></li>
    </>
  );
          ...
          <Link to="/cart-page">
            <label tabIndex={0} className="btn btn-ghost  btn-circle lg:flex items-center justify-center mr-3">
              <div className="indicator">
                ...
                <span className="badge badge-sm indicator-item bg-red text-white">{cart.length || 0}</span>
              </div>
            </label>
          </Link>
          {user ? (
